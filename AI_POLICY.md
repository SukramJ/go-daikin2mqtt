...
go-daikin2mqtt. It is modeled on the
...
gates: `make check`, which runs `go vet ./...`, a `gofumpt` formatting check,
`golangci-lint run ./...`, and `go test -race ./...` — the same checks CI enforces on every pull
request. AI assistance does not lower the bar.

## How this project itself uses AI

In the interest of transparency: go-daikin2mqtt is partly developed with AI assistance
