...
# go-daikin2mqtt — developer Makefile
#
...
BIN_DIR  := bin
MODULE   := github.com/SukramJ/go-daikin2mqtt
...
RELEASE_PAYLOAD  := characteristics.yaml config-template.yaml README.md LICENSE changelog.md
...
.PHONY: build-daemon
build-daemon: ## build the daikin2mqtt daemon
	@mkdir -p $(BIN_DIR)
	$(GO) build $(GO_BUILD_FLAGS) -o $(BIN_DIR)/daikin2mqtt ./cmd/daikin2mqtt

.PHONY: build-util
build-util: ## build the daikin2mqtt-util interactive CLI
	@mkdir -p $(BIN_DIR)
	$(GO) build $(GO_BUILD_FLAGS) -o $(BIN_DIR)/daikin2mqtt-util ./cmd/daikin2mqtt-util

.PHONY: install
install: ## go install both binaries to $(go env GOPATH)/bin
	$(GO) install $(GO_BUILD_FLAGS) ./cmd/daikin2mqtt
	$(GO) install $(GO_BUILD_FLAGS) ./cmd/daikin2mqtt-util
...
run: build-daemon ## run the daemon against ./config.yaml
	$(BIN_DIR)/daikin2mqtt --config ./config.yaml
...
release: ## stage cross-compiled release archives + notes into dist/ (no upload)
	...
	for tgt in $(RELEASE_TARGETS); do \
	  goos=$${tgt%/*}; goarch=$${tgt#*/}; \
	  stage="$(DIST_DIR)/go-daikin2mqtt-$$version-$$goos-$$goarch"; \
	  mkdir -p "$$stage"; \
	  echo "==> $$goos/$$goarch -> $$stage"; \
	  GOOS=$$goos GOARCH=$$goarch $(GO) build -trimpath -ldflags="$$ldflags" \
	    -o "$$stage/daikin2mqtt" ./cmd/daikin2mqtt; \
	  GOOS=$$goos GOARCH=$$goarch $(GO) build -trimpath -ldflags="$$ldflags" \
	    -o "$$stage/daikin2mqtt-util" ./cmd/daikin2mqtt-util; \
