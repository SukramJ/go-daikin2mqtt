...
#
# go-daikin2mqtt — Linux installer.
#
# Downloads a release tarball from GitHub, verifies its SHA-256
# checksum, installs the binaries under /opt/go-daikin2mqtt, creates a
# dedicated `daikin` system user, runs an interactive wizard for the
...
#
#   curl -sSfL https://raw.githubusercontent.com/SukramJ/go-daikin2mqtt/main/script/install.sh | sudo bash -s -- 1.2.3
#
...
REPO="${REPO:-SukramJ/go-daikin2mqtt}"
PREFIX="${PREFIX:-/opt/go-daikin2mqtt}"
CONFIG_DIR="${CONFIG_DIR:-/etc/go-daikin2mqtt}"
CONFIG_FILE="${CONFIG_FILE:-${CONFIG_DIR}/config.yaml}"
BIN_LINK_DIR="${BIN_LINK_DIR:-/usr/local/bin}"
SERVICE_USER="${SERVICE_USER:-daikin}"
SERVICE_GROUP="${SERVICE_GROUP:-daikin}"
SERVICE_NAME="${SERVICE_NAME:-go-daikin2mqtt}"
...
ARCHIVE="go-daikin2mqtt-${VERSION}-linux-${ARCH}.tar.gz"
...
WORK="$(mktemp -d -t go-daikin2mqtt-install.XXXXXX)"
...
STAGE="${WORK}/go-daikin2mqtt-${VERSION}-linux-${ARCH}"
[ -x "${STAGE}/daikin2mqtt" ] || die "extracted tree is missing daikin2mqtt binary"
...
if id -u "$SERVICE_USER" >/dev/null 2>&1; then
	...
	useradd --system --no-create-home --shell /usr/sbin/nologin \
		--user-group --comment "go-daikin2mqtt daemon" "$SERVICE_USER"
...
install -m 0755 -o root -g root "${STAGE}/daikin2mqtt"          "${PREFIX}/daikin2mqtt"
install -m 0755 -o root -g root "${STAGE}/daikin2mqtt-util"     "${PREFIX}/daikin2mqtt-util"
install -m 0644 -o root -g root "${STAGE}/characteristics.yaml" "${PREFIX}/characteristics.yaml"
...
# Symlinks for PATH convenience — daikin2mqtt-util in particular is more
# useful when it's just a `daikin2mqtt-util` away.
ln -sf "${PREFIX}/daikin2mqtt"      "${BIN_LINK_DIR}/daikin2mqtt"
ln -sf "${PREFIX}/daikin2mqtt-util" "${BIN_LINK_DIR}/daikin2mqtt-util"
...
if [ -f "$CONFIG_FILE" ]; then
	...
	CLIENT_ID="$(prompt 'Daikin ONECTA client ID' '')"
	CLIENT_SECRET="$(prompt 'Daikin ONECTA client secret' '')"
...
# Generated by go-daikin2mqtt install.sh ${VERSION}
...
[Unit]
Description=Daikin ONECTA cloud MQTT bridge (${SERVICE_NAME})
...
User=${SERVICE_USER}
Group=${SERVICE_GROUP}
ExecStart=${PREFIX}/daikin2mqtt --config ${CONFIG_FILE}
Restart=on-failure
RestartSec=5
TimeoutStopSec=10
...
${GREEN}${BOLD}install complete${RESET} — go-daikin2mqtt ${VERSION} (linux/${ARCH})

  binaries     ${PREFIX}/{daikin2mqtt,daikin2mqtt-util}
  symlinks     ${BIN_LINK_DIR}/{daikin2mqtt,daikin2mqtt-util}
  config       ${CONFIG_FILE}
  unit         ${SYSTEMD_UNIT}
...
useful commands:
  ...
  daikin2mqtt-util                              # interactive ONECTA CLI
